...
def strategy_callback(n_clicks, *flt_args):
    ...
    dd.market_sfup(clear, *flt_args)
    cte = dd.market_scte()

    vals = dd.market_sfv()
    opts = dd.market_sfo(cte)
    return vals + opts
...
def mkt_intermediary(
    ...
    dd.market_mfup(clear, *args)
    cte = dd.market_mcte(strategy_id)
    tbl_rows = dd.market_table(cte)
    for r in tbl_rows:
        r['id'] = r['market_id']  # assign 'id' so market ID set in row ID read in callbacks
    vals = dd.market_mfv()
    opts = dd.market_mfo(cte)
    n = dd.betting_db.session.query(cte).count()
